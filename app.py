...
=========================================
- Serves index.html from Flask itself (no separate static server needed)
- Uses PostgreSQL when DATABASE_URL env var is set (Render), else SQLite locally
...
# If DATABASE_URL is set (Render PostgreSQL), use it. Otherwise use local SQLite.
...
def get_conn():
    ...
    # SQLite fallback (local dev, or Render without DATABASE_URL set)
...
def save_message(session_id: str, role: str, content: str):
    ...
    if USE_POSTGRES:
        cur.execute(
            "INSERT INTO sessions (id, created) VALUES (%s,%s) ON CONFLICT DO NOTHING",
            (session_id, now))
        cur.execute(
...
def get_history(session_id: str, limit: int = 20) -> list:
    ...
    cur.close()
    conn.close()
    return list(reversed(rows))   # chronological order for Claude
...
def faqs():
    return jsonify({"faqs": [
        {"q": "How long does shipping take?",
         "a": "Standard 3–5 days. Express 1–2 days (₹99). Free above ₹999."},
        ...
        {"q": "What payment methods are accepted?",
         "a": "UPI, cards, net banking, EMI on orders above ₹2,000."},
        {"q": "What sizes are available?",
         "a": "All straps adjust from 80 cm to 145 cm."},
    ]})


# ── Init DB on startup (works for both gunicorn workers and local dev) ─────────
init_db()
