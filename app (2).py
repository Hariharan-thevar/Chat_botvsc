...
def chat():
    ...
    # Call Gemini with automatic retry for transient errors (503 high demand, 429 rate limit)
    import time
    MAX_RETRIES = 3
    bot_reply   = None
        ...
        try:
            ...
            if code == 429:
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    continue
                return jsonify({
                    "error": "Gemini is busy right now. Please wait a few seconds and try again. "
            ...
            # 503 high demand or other server error — retry automatically
            if attempt < MAX_RETRIES - 1:
                time.sleep(2 ** attempt)       # 1s, 2s, 4s
                continue
    ...
    if bot_reply is None:
        return jsonify({"error": "No response from Gemini after retries. Please try again."}), 503
